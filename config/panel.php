<?php

return [

    /*
    |--------------------------------------------------------------------------
    | App Information
    |--------------------------------------------------------------------------
    */
    'organization' => 'Jharkhand Engineering Services Association',
    'organization_hindi' => 'झारखंड अभियंत्रण सेवा संघ',
    'organization_label' => 'An Association of Engineers under Government of Jharkhand',
    'portal_name' => 'JESA PORTAL',
    // 'app_name' => 'Elloty Electronic Document Management System',
    'app_name' => 'State-level Engineer & Organization Management Panel',
    'app_sub_title' => 'JESA Administration',
    'logo' => 'img/jesa_logo.png',
    'stategovermentLogo' => 'assets/logo.png',
    'faviconIcon' => 'img/favicons/apple-touch-icon.png',
    'patrnterLogo' => 'img/computered-logo.png',
    'calendarType' => 'financialYear', // calendarYear,
    'password_expiry_days' => 30,
    'theme' => 'royal-purple',
    'themes' => [
        'gov-indigo' => [
            'primary-color' => '#4f46e5',
            'primary-hover' => '#2d2774',
            'sidebar-bg' => '#111827',
            'sidebar-secondary' => '#294786',
            'sidebar-hover' => '#1f2937',
            'sidebar-active' => '#4f46e5',
            'sidebar-active-secondary' => '#a1a7e6',
        ],

        'forest-green' => [
            'primary-color' => '#15803d',
            'primary-hover' => '#1a7a3f',
            'sidebar-bg' => '#022c22',
            'sidebar-secondary' => 'rgb(32, 143, 80)',
            'sidebar-hover' => '#064e3b',
            'sidebar-active' => '#15803d',
            'sidebar-active-secondary' => '#4ade80',
        ],

        'teal-admin' => [
            'primary-color' => '#0f766e',
            'primary-hover' => '#115e59',
            'sidebar-bg' => '#065755',
            'sidebar-secondary' => 'rgb(35, 88, 57)',
            'sidebar-hover' => '#134e4a',
            'sidebar-active' => '#0f766e',
            'sidebar-active-secondary' => '#5eead4',
        ],

        'royal-purple' => [
            'primary-color' => '#6d28d9',
            'primary-hover' => '#5b21b6',
            'sidebar-bg' => '#2e1065',
            'sidebar-secondary' => '#482883',
            'sidebar-hover' => '#3b0764',
            'sidebar-active' => '#6d28d9',
            'sidebar-active-secondary' => '#c4b5fd',
        ],

        'navy-govt' => [
            'primary-color' => '#1e3a8a',
            'primary-hover' => '#1e40af',
            'sidebar-bg' => '#020617',
            'sidebar-secondary' => '#122eac',
            'sidebar-hover' => '#020617',
            'sidebar-active' => '#1e3a8a',
            'sidebar-active-secondary' => '#93c5fd',
        ],

        'olive-admin' => [
            'primary-color' => '#3f6212',
            'primary-hover' => '#365314',
            'sidebar-bg' => '#1a2e05',
            'sidebar-secondary' => '#497c13',
            'sidebar-hover' => '#1a2e05',
            'sidebar-active' => '#3f6212',
            'sidebar-active-secondary' => '#bef264',
        ],

        'charcoal-modern' => [
            'primary-color' => '#0f172a',
            'primary-hover' => '#020617',
            'sidebar-bg' => '#000000',
            'sidebar-secondary' => '#363131',
            'sidebar-hover' => '#020617',
            'sidebar-active' => '#334155',
            'sidebar-active-secondary' => '#64748b',
        ],
    ],
];
