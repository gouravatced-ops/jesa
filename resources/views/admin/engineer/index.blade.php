...
@section('title', 'Engineer List | JESA')
...
<div class="card">
    ...
    <div class="card-head">
        ...
        <div class="card-actions">
            <form method="GET" action="{{ route('admin.engineers.index') }}" class="search-box" onsubmit="return false;">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input type="text" id="engineerSearchInput" name="search" value="{{ $search }}" placeholder="Search engineers..." autocomplete="off">
            </form>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <a class="action-btn edit" href="{{ route('admin.engineers.edit', $engineer->encrypted_route_key) }}" title="Edit">
    ...
    @if($engineers->total() > 0)
        <div class="table-pagination" id="engineerPagination">
            <span>
                Showing <strong>{{ $engineers->firstItem() }}</strong> to <strong>{{ $engineers->lastItem() }}</strong> of <strong>{{ $engineers->total() }}</strong> engineers
            </span>
            <div class="pagination-btns">
                ...
                @else
                    <a class="pag-btn" href="{{ $engineers->previousPageUrl() }}"><i class="fa-solid fa-chevron-left"></i></a>
                @endif

                @foreach($engineers->getUrlRange(1, $engineers->lastPage()) as $page => $url)
                    <a class="pag-btn {{ $page === $engineers->currentPage() ? 'active' : '' }}" href="{{ $url }}">{{ $page }}</a>
                @endforeach

                @if($engineers->hasMorePages())
                    <a class="pag-btn" href="{{ $engineers->nextPageUrl() }}"><i class="fa-solid fa-chevron-right"></i></a>
                @else
